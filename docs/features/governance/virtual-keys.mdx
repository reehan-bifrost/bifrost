...
- A pattern that does not compile, is blank or whitespace-only, or repeats another pattern in the same list is refused with a 400 when the configuration is saved.
