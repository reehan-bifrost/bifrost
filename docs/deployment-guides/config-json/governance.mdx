...
| `allowed_models_patterns` | No | RE2 patterns that allow models by name shape, next to `allowed_models`. Case-insensitive full match against the model name and `provider/model`; `*`, blank entries, and duplicates are refused |
| `blacklisted_models_patterns` | No | RE2 patterns that block models by name shape, matched and validated the same way; win over the allow side |
